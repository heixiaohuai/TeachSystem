$(function(){
                ...
                $.post("/TeachSystem/admin/getCourByPage",{"currPage":1},function(courses){
                    ...
                    $.each(courses,function(index,course){
                        content+="<tr id='data'>" +
                            ...
                            "<td>"+course.ckind+"</td>"+
                            "<td>"+
                            "<a href='javascript:void(0)' id='select'>选课</a>"+
                            "</td>"
                        "</tr>";
    ...
    function getId(){
        var str=window.parent.location.search;//获得整个URL地址
        var pos=str.indexOf("=");
        var id=str.substr(pos+1);
        return id;
    }

    //选课
    $(".table").on("click",".tablebd #data #select",function(){
        var mythis=$(this);
        var course_id = $(this).parents("tr#data").find("td").eq(0).text();
        //获取学生id
        $.post("/TeachSystem/getUserInfo",{"id":getId()},function(user){
            var student_id=user.student.id;

            $.post("/TeachSystem/selectcourse",{"student_id":student_id,"course_id":course_id,"flag":"选中"},function(msg){
                alert(msg);
                mythis.removeAttr("id").text("已选");
            },"text");
