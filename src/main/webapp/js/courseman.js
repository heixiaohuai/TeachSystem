$(function(){
	...
	$(".toolbar .optarea .add").click(function(){
		$(".addstu").show();
        $.post("/TeachSystem/admin/getAllTeacher",{},function (teachers) {
            var content = "";
            $.each(teachers,function (index,teacher) {
                content+="<option value=" + teacher.tid + ">" + teacher.tname + "</option>";
            });
            $("#addform #cteacher").empty().append(content);
        },"json");
	});
	...
	$(".table").on("click",".tablebd tr#data a#update",function () {
        ...
        $.post("/TeachSystem/admin/getAllTeacher",{},function (teachers) {
            ...
            $("#updateform #uteacher").empty().append(content);
        },"json");
        var arr=[];
   ...
   //修改教师信息表单验证
    ...
    function showUsers(){
        $.post("/TeachSystem/admin/getCourTotalRows",{},function(count){
            $.post("/TeachSystem/admin/getCourTotalPages",{},function(pagestotal){
                //显示第一页数据
                $.post("/TeachSystem/admin/getCourByPage",{"currPage":1},function(courses){
                    console.log(courses);
                    var thead="";
                    var content="";
                    $.each(courses,function(index,course){
                        ...
                        thead+="<tr id='head"+index+"'>"+
                            "<td> \
                            <input type='checkbox' name='head"+index+"' id='head"+index+"' myid='"+course.cid+"'>"+
                            "</td> \
                                    </tr>";
                    });
                ...
                $("#page").paging({
                    ...
                    callback: function (num) { //回调函数
                        ...
                        $.post("/TeachSystem/admin/getCourByPage",{"currPage":num},function(courses){
                            var thead="";
                            var content="";
                            $.each(courses,function(index,course){
                                content+="<tr id='data'>" +
    ...
    $(".table").on("click",".tablebd #data #delete",function(){
        ...
        var delTeacher = $(this).parents("tr#data").find("td").eq(4).text();
        var delName = $(this).parents("tr#data").find("td").eq(2).text();
        var flag = confirm("确定要删除"+delTeacher+"老师的"+delName+"的课程信息吗？");
        if (flag){
            $.post("/TeachSystem/admin/deleteCourById",{"did":did},function(msg){
                window.location.href="/TeachSystem/admin/courseman.html";
            },"text");
        }
        return false;
    });
    ...
    $(".toolbar .optarea .delete").click(function(){
        ...
        if (flag){
            $.post("/TeachSystem/admin/deleteCourByIds",{"myids":myids},function(msg){
                window.location.href="/TeachSystem/admin/courseman.html";
            },"text");
        }
    });
